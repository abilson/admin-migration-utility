...
namespace AdminMigrationUtility.CustomPages.Models
{
	public class ManagerQueueRecordModel
	{
		...
		[Required(ErrorMessage = Constant.Messages.Agent.PRIORITY_REQUIRED)]
		...
		[Required(ErrorMessage = Constant.Messages.Agent.ARTIFACT_ID_REQUIRED)]
		...
		public ManagerQueueRecordModel(DataRow row, ISqlQueryHelper sqlQueryHelper)
		{
			SetPropertiesFromDataRow(row, sqlQueryHelper);
		}

		private void SetPropertiesFromDataRow(DataRow row, ISqlQueryHelper sqlQueryHelper)
		{
